    ...
    try:
        while True:
            ...
            if correct_answers == NUM_QUESTIONS:
                ...
                vprint(
                    f"rejected {correct_answers}", file=process.stdin, verbose=verbose
                )
            submissions += 1
...
def main() -> int:
    ...
    with open(args.data, "r") as data:
        ...
        if len(answer) != NUM_QUESTIONS:
            raise ValueError(
                f"Invalid input: answer string must have length {NUM_QUESTIONS}"
            )
        if not re.fullmatch("[tf]+", answer):
            raise ValueError(
                "Invalid input: answer string must contain only `t` or `f`"
            )
